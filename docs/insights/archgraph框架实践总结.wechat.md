...
- **上层：指挥官与参谋** — 人类用户定义意图、验收决策（指挥官）；图谱审计员 Agent（如 Derek 语义审计员、John 语法审计员）作为参谋，审计图谱质量、发现问题
- **中层：知识图谱** — 单一事实来源，包含 Work Package、Skill、Rule、Business Actor、长期记忆等
- **下层：执行层士兵** — Developer、产品经理、架构师、设计师、发布员等所有 Worker Agent 照着图谱行动，并将经验写回图谱

**关键点**：产品经理、架构师、规划专家等都是「士兵」，他们执行具体任务；「参谋」是专门审计图谱质量的 Agent，确保图谱的正确性和一致性。
