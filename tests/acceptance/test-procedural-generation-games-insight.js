#!/usr/bin/env node
/**
 * Acceptance test for Work Package 3000
 * 「洞察自由生成的游戏（场景不固化）的现状与技术趋势」
 *
 * GIVEN  工作包「洞察自由生成的游戏（场景不固化）的现状与技术趋势」已被领取并产出洞察交付物
 * WHEN   验收方打开洞察交付物文档 docs/insights/自由生成游戏-程序化内容生成-洞察.md
...
function main() {
  ...
  // 0) The graph must register this acceptance test on Work Package 3000.
  let graph = null;
  try {
  ...
  record('工作包「洞察自由生成的游戏（场景不固化）的现状与技术趋势」存在', !!wp);
  if (wp) {
    const tc = (wp.testcases || []).find((t) => t.type === 'Acceptance Test');
    record('工作包挂载可执行验收用例', !!tc);
    if (tc) {
      const refersThisFile = (tc.acceptanceCriteria || '').includes('test-procedural-generation-games-insight.js');
